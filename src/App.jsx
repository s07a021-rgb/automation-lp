...
const C = {
  ...
  borderHov:    "hover:border-stone-300",
  accent:       "text-green-700",
  accentBg:     "bg-green-600",
  accentHov:    "hover:bg-green-700",
  accentShadow: "shadow-green-600/20",
...
function UrgencyBadge() {
  return (
    <p className="inline-flex items-center gap-2 text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-200 rounded-full px-4 py-1.5 mb-5">
...
function CtaButton({ label = "無料診断はこちら（3分・営業連絡なし）", size = "lg" }) {
  const base = `block w-full sm:w-auto text-center ${C.accentBg} text-white font-bold ${C.accentHov} active:scale-95 transition-all shadow-lg ${C.accentShadow} rounded-2xl`;
...
function CtaTrust({ center = true }) {
  return (
    <div className={`mt-3 ${center ? "text-center" : ""}`}>
      <p className="text-sm text-green-700 font-medium">最短即日でご提案いたします</p>
      <p className="text-xs text-stone-500 mt-1">記入3分・費用ゼロ・営業連絡なし</p>
    </div>
  );
}
...
function InlineCta({ title, sub }) {
  return (
    <FadeIn>
      <div className="card-highlight my-4 border border-green-200 bg-green-50 rounded-3xl px-6 py-10 text-center">
        <UrgencyBadge />
        <p className="font-bold text-lg mb-2 text-stone-800">{title}</p>
        <p className="text-stone-500 text-sm mb-7">{sub}</p>
...
const CASES = [
  ...
  {
    industry: "製造業・従業員500名",
    before: "紙・Excelで出荷指示と在庫確認を実施し、部門ごとに管理が分かれていた",
    after: "出荷データから自動で在庫引当・指示を生成し、全体でリアルタイム管理",
    result: "作業時間 ▲60% ／ 出荷ミス大幅減",
...
function FaqItem({ faq, delay }) {
  ...
  return (
    <FadeIn delay={delay}>
      <button
        onClick={() => setOpen((v) => !v)}
        className={`w-full text-left ${C.card} border ${C.border} ${C.borderHov} rounded-2xl px-6 py-5 transition-all shadow-sm hover:shadow-md`}
      >
        <div className="flex items-center justify-between gap-4">
          <span className="font-semibold text-sm md:text-base leading-snug text-stone-800">{faq.q}</span>
          <ChevronDown
            size={18}
            className={`shrink-0 text-green-600 transition-transform duration-300 ${open ? "rotate-180" : ""}`}
          />
        </div>
...
function PrivacyModal({ onClose }) {
  return (
    ...
    >
      <div
        className={`${C.card} border ${C.border} rounded-3xl max-w-lg w-full max-h-[80vh] overflow-y-auto p-8 relative shadow-xl`}
      ...
      >
        ...
        <h2 className="text-lg font-bold mb-6 text-stone-800">プライバシーポリシー</h2>

        <section className="mb-5">
          <h3 className="text-sm font-semibold text-green-700 mb-2">1. 個人情報の利用目的</h3>
          <p className="text-stone-500 text-sm leading-relaxed">
            お問い合わせフォームを通じていただいた氏名・メールアドレス・会社名等の個人情報は、
            以下の目的にのみ使用します。
          </p>
          <ul className="text-stone-500 text-sm leading-relaxed mt-2 space-y-1 pl-4 list-disc">
        ...
        <section className="mb-5">
          <h3 className="text-sm font-semibold text-green-700 mb-2">2. 第三者への提供</h3>
          <p className="text-stone-500 text-sm leading-relaxed">
        ...
        <section className="mb-5">
          <h3 className="text-sm font-semibold text-green-700 mb-2">3. 個人情報の管理</h3>
          <p className="text-stone-500 text-sm leading-relaxed">
        ...
        <section className="mb-5">
          <h3 className="text-sm font-semibold text-green-700 mb-2">4. 個人情報の開示・訂正・削除</h3>
          <p className="text-stone-500 text-sm leading-relaxed">
        ...
        <section>
          <h3 className="text-sm font-semibold text-green-700 mb-2">5. お問い合わせ先</h3>
          <p className="text-stone-500 text-sm leading-relaxed">
            個人情報の取り扱いに関するお問い合わせは下記までご連絡ください。
          </p>
          <p className="text-sm mt-2 text-stone-700">
            合同会社More Agri<br />
            <a href="mailto:info@more-agri.com" className="text-green-700 hover:underline">
              info@more-agri.com
            </a>
          </p>
        </section>
...
function StickyCTA() {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-50 transition-transform duration-300 ${visible ? "translate-y-0" : "translate-y-full"}`}>
      <div className="bg-white/95 backdrop-blur border-t border-stone-200 shadow-lg px-4 py-3 flex items-center justify-between gap-3 max-w-screen-lg mx-auto">
        <p className="text-sm text-stone-600 hidden sm:block shrink-0">
          お気軽にご相談ください。<span className="text-green-700 font-semibold">まず診断を。</span>
        </p>
        <a
          href={CTA_URL}
          target="_blank"
          rel="noopener noreferrer"
          className={`flex-1 sm:flex-none text-center ${C.accentBg} text-white px-6 py-2.5 rounded-xl font-bold text-sm ${C.accentHov} transition-colors`}
        >
          無料診断はこちら（3分・営業連絡なし）
        </a>
        <button
...
// ── 波形セクション区切り ──────────────────────────────────────────────────
function WaveDivider({ from = "#fafaf9", to = "#f5f5f4" }) {
  return (
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen ${C.bg} text-stone-700`}>

      {/* ===== HERO ===== */}
      <section className="relative px-6 py-20 md:py-28 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[760px] h-[420px] bg-green-400/8 rounded-full blur-3xl" />
          <div className="absolute top-8 left-[10%] w-[300px] h-[200px] bg-emerald-400/6 rounded-full blur-2xl" />
          <div className="absolute top-14 right-[8%] w-[260px] h-[170px] bg-teal-400/5 rounded-full blur-2xl" />
        </div>
        <div className="relative max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <FadeIn delay={80}>
                <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-4 text-stone-900">
                  その事務作業、<br />
                  <span className="text-green-700">まだ手でやっていますか？</span>
                </h1>
              </FadeIn>
              <FadeIn delay={140}>
                <p className="text-xl md:text-2xl font-semibold text-stone-800 mb-4">
              ...
              <FadeIn delay={280}>
                ...
                <CtaTrust center={false} />
                <div className="flex flex-wrap gap-2 mt-5">
                  {["即日対応可", "既存システムそのまま利用可", "初期費用0円"].map((badge) => (
                    <span key={badge} className={`text-xs text-stone-500 ${C.card} border ${C.border} rounded-full px-3 py-1 shadow-sm`}>
                      ✓ {badge}
                    </span>
                  ))}
                </div>
      ...
      <section className="px-6 py-12">
        <div className="max-w-3xl mx-auto">
          <FadeIn>
            <h2 className="text-xl md:text-2xl font-bold text-center mb-6 text-stone-700">
              こんな会社のお役に立てます
            </h2>
          </FadeIn>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <FadeIn key={title} delay={i * 80}>
                <div className={`card-highlight ${C.card} border ${C.border} rounded-2xl p-6 h-full shadow-sm hover:shadow-md transition-shadow`}>
                  <p className="text-3xl mb-3">{emoji}</p>
                  <p className="font-bold mb-2 text-stone-800">{title}</p>
          ...
          <FadeIn>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {SOCIAL_PROOF.map(({ num, label }) => (
                <div key={label} className={`${C.card} border ${C.border} rounded-2xl p-4 text-center shadow-sm`}>
                  <p className="text-xl font-bold text-green-700">{num}</p>
        ...
        <div className="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <FadeIn key={label} delay={i * 80}>
              <div className={`${C.card} border ${C.border} hover:border-green-300 rounded-2xl p-5 text-center transition-all shadow-sm hover:shadow-md`}>
                <p className="text-2xl font-bold text-green-700">{num}</p>
                <p className="text-sm font-semibold mt-1 text-stone-800">{label}</p>
      ...
      <WaveDivider from="#fafaf9" to="#f5f5f4" />

      {/* ===== PAIN ===== */}
      <section className={`px-6 py-16 ${C.bgAlt} dot-bg`}>
        <div className="max-w-3xl mx-auto">
          <FadeIn>
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-3 text-stone-800">
              その手作業、一緒になくしていきませんか？
            </h2>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {PAIN_POINTS.map((point, i) => (
              <FadeIn key={point} delay={i * 60}>
                <li className={`flex items-start gap-3 ${C.card} border ${C.border} rounded-2xl px-5 py-4 shadow-sm`}>
          ...
          <FadeIn delay={400}>
            <p className="text-center text-green-700 font-semibold mt-8 mb-10">
      ...
      <WaveDivider from="#f5f5f4" to="#fafaf9" />

      {/* ===== FLOW ===== */}
      <section className="px-6 py-16 relative">
        <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gradient-to-b from-transparent via-green-400/20 to-transparent" />
        <div className="max-w-3xl mx-auto">
          <FadeIn>
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-3 text-stone-800">
              導入の流れ
                ...
                <FadeIn key={step.num} delay={i * 120}>
                  <div className="flex gap-5">
                    <div className="flex flex-col items-center shrink-0">
                      <div className={`w-12 h-12 rounded-full ${C.accentBg} text-white font-bold flex items-center justify-center shadow-md shadow-green-600/20`}>
                        <Icon size={20} />
                      </div>
                      {i < STEPS.length - 1 && (
                        <div className="w-px h-12 bg-gradient-to-b from-green-400/40 to-transparent my-1" />
                      )}
                    </div>
                    <div className="pb-10">
                      <p className="text-xs font-mono text-green-700 mb-0.5">{step.num}</p>
                      <p className="font-bold text-lg mb-1 text-stone-800">{step.title}</p>
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                      <div className={`${C.cardMid} rounded-xl p-4`}>
                        <p className="text-xs text-stone-400 mb-1.5 font-medium">導入前</p>
                        <p className="text-sm text-stone-600 leading-relaxed">{c.before}</p>
                      </div>
                      <div className={`${C.cardMid} rounded-xl p-4`}>
                        <p className="text-xs text-stone-400 mb-1.5 font-medium">導入後</p>
                        <p className="text-sm text-stone-600 leading-relaxed">{c.after}</p>
                      </div>
                    </div>
                    <p className="text-sm font-bold text-green-700 bg-green-50 border border-green-200 rounded-xl px-4 py-2 inline-block">
                      {c.result}
                    </p>
                  </div>
                </div>
      ...
      <section className="px-6 py-16 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-[260px] bg-green-500/6 rounded-full blur-3xl" />
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-center">
              ...
              <div className="order-1 sm:order-2">
                ...
                <p className="text-stone-500 text-sm leading-relaxed mb-6">
                  「何から始めればいいかわからない」という方が一番多いです。<br />
                  30分のヒアリングで、自動化できる箇所と効果の概算をその場でお伝えします。
                </p>
                <CtaButton size="md" />
                <CtaTrust center={false} />
              </div>
      ...
      {/* ===== FINAL CTA ===== */}
      <section className="px-6 py-20 relative overflow-hidden bg-gradient-to-b from-green-50 to-stone-50">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[460px] h-[280px] bg-green-500/8 rounded-full blur-3xl" />
        </div>
        <div className="relative max-w-xl mx-auto text-center">
          <FadeIn>
            <UrgencyBadge />
            <h2 className="text-2xl md:text-3xl font-bold mb-4 text-stone-800">
              一緒に、業務をラクにしていきませんか。
            </h2>
            <p className="text-stone-500 text-sm mb-8 leading-relaxed">
          ...
          <FadeIn delay={80}>
            <div className="grid grid-cols-3 gap-3 mb-8">
              {REASSURANCES.map(({ icon: Icon, text, sub }) => (
                <div key={text} className={`${C.card} border ${C.border} rounded-2xl p-4 shadow-sm`}>
                  <Icon size={18} className="text-green-600 mx-auto mb-2" />
                  <p className="text-xs font-bold mb-1 text-stone-800">{text}</p>
          ...
          {/* CTA⑤ フッター前 */}
          <FadeIn delay={160}>
            <p className="text-stone-600 text-sm font-medium mb-5">
              迷っているうちに、受付枠が埋まります。<br />
              <span className="text-stone-800 font-semibold">まず話を聞くだけでも構いません。</span>
            </p>
            <div className="flex justify-center">
              <CtaButton />
            </div>
      ...
      {/* ===== 信頼一文 ===== */}
      <div className={`px-6 py-4 ${C.bgAlt} border-t border-stone-200 text-center`}>
      ...
      {/* ===== FOOTER ===== */}
      <footer className={`${C.bgAlt} border-t border-stone-200 px-6 py-10`}>
        <div className="max-w-3xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="text-center sm:text-left">
            <p className="font-bold text-stone-800 text-sm">合同会社More Agri</p>
            <a
              href="mailto:info@more-agri.com"
              className="text-stone-400 text-xs hover:text-green-700 transition-colors mt-1 block"
            >
              info@more-agri.com
            </a>
          </div>
